...
ROOT_FOLDER_PATH = "/Users/lfarz/Desktop/לימודים/תואר שני/מעבדות מתקדמות בAI/face-blocker/face-blocker/Siamese_network/imgs/"
SHOW_3D_PLOT = False
...
TEST_IMAGES_FOLDER = "/Users/lfarz/Desktop/לימודים/תואר שני/מעבדות מתקדמות בAI/face-blocker/face-blocker/test_imgs"
...
class FaceLandmarksDataset(Dataset):
    def __init__(self, pairs, labels, transform=None):
        print(f"Initializing dataset with {len(pairs)} pairs and {len(labels)} labels")
        self.pairs = pairs
        self.labels = labels
    ...
    def __getitem__(self, idx):
        ...
        # Ensure points are numpy arrays or tensors
        points1 = torch.tensor(points1, dtype=torch.float32)
        points2 = torch.tensor(points2, dtype=torch.float32)
        label = torch.tensor(label, dtype=torch.float32)

        # Add channel dimension to match Conv1d expected input: [channels, length]
        points1 = points1.transpose(0, 1)  # Transpose to get [3, number_of_points]
        points2 = points2.transpose(0, 1)  # Same for points2
...
class EmbeddingModel(nn.Module):
    def __init__(self):
        ...
        # Initialize the 1D convolutional layers
        ...
        self.pool = nn.MaxPool1d(2)  # Max pooling over a (2) window

        # Dummy input to calculate flattened size
        dummy_input = torch.rand(1, 3, 478)  # Adjust 478 to your actual points length
        ...
        self.fc3 = nn.Linear(800, 650)
        self.fc4 = nn.Linear(
            650, 512
        )  # The final output is a 512-dimensional feature vector
...
class SiameseNetwork(nn.Module):
    def __init__(self, embedding_model):
        ...
        self.l1_dist = L1Dist()
        # Assuming `fc4` is the last layer in the embedding model with an output size of 512
        self.classifier = nn.Sequential(
            nn.Linear(512, 1),  # Use the output size directly from the last FC layer
            nn.Sigmoid(),
        )

    def forward(self, x1, x2):
        embed1 = self.embedding_model(x1)
        embed2 = self.embedding_model(x2)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for (test_points1, test_points2), test_labels in test_loader:
                ...
                test_outputs = model(test_points1, test_points2)
...
def save_vectors_to_file(vectors, filename):
    """Save vectors to a file securely."""
    print(f"Saving vectors to file: {filename}")
    try:
        np.save(filename, vectors, allow_pickle=True)
        print(f"Vectors saved successfully to {filename}")
    except Exception as e:
        print(f"Failed to save vectors: {e}")


def load_vectors_from_file(filename):
    """Load vectors from a file with comprehensive error handling."""
    print("Attempting to load vectors from file:", filename)
    try:
        ...
        print("Error: Load operation receiving invalid data.")
    except EOFError as e:
        print("EOF Error encountered:", e)
    except pickle.UnpicklingError:
        print("Error: Data could not be unpickled. The file may be corrupted.")
    return None
...
def rotate_3d_points(points, embedding, positive_images=False):
    ...
    if positive_images:
        ...
        for x_angle in range(10, 91, 10):
            for y_angle in range(-30, 31, 10):
...
def plot_rotated_3d_points(ax, points, rotated_points):
    ...
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.setTitle("3D Points Rotation")
    ax.view_init(elev=-90, azim=270)
    plt.draw()
    plt.pause(0.00001)
...
def normalize_points(points, target_distance=100.0, reference_index=8):
    ...
    # multiply by 1000 to scale the points
    points_centered = points_centered * 1000
...
def main(root_folder_path):
    ...
    if CREATE_NEW_DATA:
        ...
        negative_landmarks = process_folder_images(
            negatives_folder, mp_face_mesh, facenet_model, max_files=10000
        )
...
def calculate_threshold(model, positive_vectors, max_samples=100):
    """Calculate a decision threshold based on a sample of positive vectors."""
    print_blue("Calculating threshold...")

    # Ensure the positive_vectors is a list, if not convert it
    if not isinstance(positive_vectors, list):
        positive_vectors = list(positive_vectors)

    # Ensure that there are enough vectors for sampling
    num_samples = min(max_samples, len(positive_vectors))
    if num_samples < 2:
        print_red("Not enough samples to calculate the threshold.")
        return None

    threshold_output = []
    sampled_vectors = random.sample(positive_vectors, num_samples)

    # Double loop to compare each pair
    for sample1 in sampled_vectors:
        for sample2 in sampled_vectors:
            model.eval()
            with torch.no_grad():
                # Ensure inputs are correctly shaped [batch_size, channels, length]
                sample1_tensor = (
                    torch.tensor(sample1, dtype=torch.float32).to(device).view(1, 3, -1)
                )
                sample2_tensor = (
                    torch.tensor(sample2, dtype=torch.float32).to(device).view(1, 3, -1)
                )
                output = model(sample1_tensor, sample2_tensor)
                threshold_output.append(output.item())

    # Find the threshold that is greater than 65% of all outputs
    threshold_output.sort()
    threshold = threshold_output[int(len(threshold_output) * 0.95)]
    print_green(f"Threshold calculated: {threshold}")
    return threshold


def compare_vectors(model, points, vectors):
    """
    Compare the input points with a list of vectors using the trained model.

    Args:
        model (nn.Module): Trained Siamese network model.
        points (np.ndarray): Array of 3D points.
        vectors (list): List of vectors to compare with.

    Returns:
        list: List of distances between the input points and vectors.
    """
    results = []
    model.eval()
    with torch.no_grad():
        for vector in vectors:
            # Ensure inputs are correctly shaped [batch_size, channels, length]
            vector_tensor = (
                torch.tensor(vector, dtype=torch.float32).to(device).view(1, 3, -1)
            )
            points_tensor = (
                torch.tensor(points, dtype=torch.float32).to(device).view(1, 3, -1)
            )
            output = model(points_tensor, vector_tensor)
...
def test_image(
    ...
    print_blue("Testing image...")

    points, embedding = process_image(image_path, mp_face_mesh, facenet_model)
    if points is not None and embedding is not None:

        results = []
        # Compare the normalized points with the positive and negative vectors
        positive_results = compare_vectors(model, points, positive_vectors)
        # negative_results = compare_vectors(model, normalized_points, negative_vectors)
        ...
        print(f"Positive results: {results}")
        print(f"len(positive_results): {len(positive_results)}")
        persentage_positive = sum(results) / len(positive_results)
        print(f"persentage_positive: {persentage_positive}")

        if persentage_positive > 0.5:
            print_green("The person in the image is a match.")
            heder = "Match"
            color = (0, 255, 0)
        else:
            print_red("The person in the image is not a match.")
            heder = "Not a Match"
            color = (0, 0, 255)

        # Plot the image with the results as heder in the same color red or green
        image = load_image(image_path)
        # convert the image to RGB (OpenCV uses BGR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        image = cv2.putText(
            image, heder, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA
        )
        ...
        print("Landmarks could not be detected in the image.")
...
if __name__ == "__main__":
    ...
    model = load_trained_model(PATH_FOR_MODEL)
    mp_face_mesh = load_mesh_model()
    facenet_model = load_facenet_model()
